...
from lib.encoders import QueryPipeline, TextEncoderPipeline
...
def update_database(data, batch_id):
    ...
    collection.create_index(
        "embeddings", {"index_type": "FLAT", "metric_type": "IP", "params": {}}
    )
...
# Create a file request class with file name and text
...
@app.post("/data_upload/", response_model=FileRequestResponse)
def data_upload(data: List[FileRequest]):
    # TODO improve batch ID system - use user specified batch name
    # batch_id = data["batch_id"]
    batch_id = random.randint(1000, 9999)
...
@app.get("/search/")
def search(batch_id: int, query: str, filepath: Optional[str] = None):
    if not get_batch_status(batch_id):
        # return 404
        return {"status": "error", "message": "batch id not found"}, 404
